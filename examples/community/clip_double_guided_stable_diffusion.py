...
class CLIPGuidedStableDiffusion(DiffusionPipeline):
    ...
    def cond_fn(
        ...
        image_embeddings_clip_guidance,
        clip_guidance_scale,
        num_cutouts,
        use_cutouts=True,
    ):
    ...
    def __call__(
        ...
        if clip_guidance_scale > 0:
            ...
            if clip_image is not None:
                clip_image = transforms.Resize(self.feature_extractor.size)(clip_image)
                clip_image = self.normalize(clip_image).to(self.device)
                image_embeddings_clip = self.clip_model.get_image_features(clip_image)
                image_embeddings_clip = image_embeddings_clip / image_embeddings_clip.norm(p=2, dim=-1, keepdim=True)
                image_embeddings_clip = image_embeddings_clip.repeat_interleave(num_images_per_prompt, dim=0)
